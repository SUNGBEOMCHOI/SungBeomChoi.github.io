...
state벡터 중 관측가능한 state와 관측불가능한 state : <a href="https://www.codecogs.com/eqnedit.php?latex=\mathbf{s}_{t}=\left[\mathbf{s}_{t}^{o},&space;\mathbf{s}_{t}^{h}\right]" target="_blank"><img src="https://latex.codecogs.com/gif.latex?\mathbf{s}_{t}=\left[\mathbf{s}_{t}^{o},&space;\mathbf{s}_{t}^{h}\right]" title="\mathbf{s}_{t}=\left[\mathbf{s}_{t}^{o}, \mathbf{s}_{t}^{h}\right]" /></a>
...
A3C에서는 많은 threads에서 병렬적으로 각 환경에서 agents가 결정을 내린다. 이것을 training experiences를 빠르게 병렬로 처리하기 위해 GPU에서 실행한 것을  GA3C라고 부른다. 
...
<img src="https://latex.codecogs.com/gif.latex?v_{\text&space;{pref}}" title="v_{\text {pref}}" />: <img src="https://latex.codecogs.com/gif.latex?\pm\pi" title="\pm\pi" />/3, <img src="https://latex.codecogs.com/gif.latex?\pm\pi" title="\pm\pi" />/6, 0
...
각 GPU의 threads에서 agent는 experience(<img src="https://latex.codecogs.com/gif.latex?\left\{\mathbf{s}_{t}^{j&space;n},&space;\mathbf{u}_{t},&space;r_{t}\right\}" title="\left\{\mathbf{s}_{t}^{j n}, \mathbf{u}_{t}, r_{t}\right\}" />)를 모은다. 
